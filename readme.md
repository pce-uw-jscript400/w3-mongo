...
* **Your Answer:** MongoDB - document-oriented database program, Mongoose - most popular MongoDB object modeling tool.
...
* **Your Answer:** Database hold collections of documents, collection - may store a number of documents. Document - is the unit of storing data in a MongoDB database.
...
* **Your Answer:** Types you can save to the database. Available Data types: Number, String, Dates Arrays, Embedded documents, booleans, regular expressions
...
* **Your Answer:** If you do not include all the fields as specified, the document still will be created with missing fields (or auto generated - like timestamp)
...
* **Your Answer:** The first parameter of the updateOne() method is a query object defining which document to update. The second parameter is an object defining the new values of the document.
...
* **Your Answer:** updateOne() return meta data about object. findOneAndUpdate() return object without update.
...
* **Your Answer:** new: true. findOneAndUpdate({ _id: req.params.id }, { title: req.body.title }, { new: true })
...
* **Your Answer:** I do not see any errors. (const response = await Series.find().select('_id title start_year season_count') )
...
* **Your Answer:** With catch (error) - error message can include more details.
...
* **Your Answer:** Auto generated ID

* **Question:** With the current routes that we have, how would you upate the name of a character in a series?

* **Your Answer:** I added 'characters: req.body.characters' to existing findOneAndUpdate()
...
* **Your Answer:** After get request, to find by '/:id', so they stay in order.
